...
class GaussianPlusFlatCosThetaLSToVolumetricPriorRatio(PriorRatio):
    ...
    hyperparams = ['cos_theta_ls_mean', 'cos_theta_ls_std', 'flat_prior_percentage']
    ...
    def lnprior_ratio(self, cos_theta_ls, m1_source, z,
        ...
        gaussian_plus_flat_cos_theta_ls = \
            flat_prior_percentage * 0.5 + beta * gaussian_cos_theta_ls
...
class FixedUniformPercentageFlatPercentagePrior(FixedPrior):
    """
    Gives the hyperprior for a gaussian + uniform distribution of
    cos(theta_LS) for a fixed proportion of the flat distribution
    defined as flat_prior_percentage
    """
    standard_par_dic = {'flat_prior_percentage':0}

class CombinedRateUniformMuSigmaFlatPercentagePriors(CombinedPrior):
    """
    ...
    prior_classes = [UniformCosThetaLSRateHyperPrior,
                     ...
                     FixedUniformPercentageFlatPercentagePrior,
                     UniformCosThetaLSMeanHyperPrior]

class CombinedRateUniformSigmaFlatPercentageFixedMuPriors(CombinedPrior):
    """
    Takes the individual priors for each parameter above and constructs
